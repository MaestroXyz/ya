# WizzSec was proudly coded by Rdimo (https://github.com/Rdimo).
# Copyright (c) 2021 Rdimo#6969 | https://Cheataway.com
# WizzSec Nuker under the GNU General Public Liscense v2 (1991).
...
def main():
    setTitle(f"WizzSec Nuker {THIS_VERSION}")
    clear()
    global threads
    global cancel_key
    if getTheme() == "WizzSecous":
        banner()
    elif getTheme() == "dark":
        banner("dark")
    elif getTheme() == "fire":
    ...
    if choice == "1":
        ...
        if threading.active_count() < threads:
            threading.Thread(target=util.accountNuke.WizzSec_Nuke, args=(token, Server_Name, message_Content)).start()
            return
        ...
        if token.startswith("mfa."):
            print(f'{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] : Just a headsup WizzSec wont be able to delete the servers since the account has 2fa enabled')
            sleep(3)
        processes = []
        #get all servers
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        search_for_updates()
        with open(os.getenv("temp")+"\\WizzSec_proxies", 'w'): pass
        if not os.path.exists(os.getenv("temp")+"\\WizzSec_theme"):
            setTheme('WizzSecous')
        clear()
        proxy_scrape()
        sleep(1.5)
    ...
    try:
        ...
        print(f"{Fore.RED}Woopsie daisy, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with WizzSec nuker, please download python 3.7+")
        sleep(5)
        print("exiting. . .")
        sleep(1.5)
        os._exit(0)
